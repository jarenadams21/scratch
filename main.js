...
const rIC = cb => (typeof queueMicrotask !== "undefined" ? queueMicrotask : (f) => Promise.resolve().then(f))(cb);
const _handlers = [];
function addNotificationHandler(fn) {
    _handlers.push(fn);
}
...
function _passesPolicy(type, props) {
...
function createElement(type, props, ...children) {
    const p = Object.assign(Object.assign({}, props), { children: children
            .map(c => (c == null || c === false) ? null : typeof c === "object" ? c : createTextElement(c))
            .filter(Boolean) });
    if (!_passesPolicy(type, p)) {
        _notify(`createElement blocked: "${type}"`, "error");
        return null;
    }
    return { type, props: p };
}
...
const isPassive = (k) => k === "touchstart" || k === "touchmove" || k === "wheel" || k === "scroll";
...
function commitWork(fiber) {
    ...
    let p = fiber.parent;
    while (!p.dom)
        p = p.parent; // climb past function-component fibers
...
function runEffectCleanups(fiber) {
    if (!fiber)
        return;
    if (fiber.hooks) {
        fiber.hooks.filter((h) => h._effect && h.cleanup).forEach((h) => {
            h.cleanup();
            h.cleanup = null;
        });
    }
...
function reconcileChildren(wipFiberArg, elements) {
    elements = elements.flat().filter(Boolean); // flatten .map() arrays
    let index = 0;
    ...
    while (index < elements.length || (!hasKeys && oldFiber != null)) {
        ...
        if (elementIsKeyless && oldFiber)
            keylessConsumed.add(oldFiber);
        ...
        if (sameType) {
            ...
            if (hasKeys)
                keyMap.delete(element.props.key);
        }
        if (element && !sameType) {
...
// DESIGN SYSTEM
// ═══════════════════════════════════════════════════════════════════════════
engine.addNotificationHandler((msg, sev) => ({ info: console.log, alert: console.warn, error: console.error })[sev](`[engine:${sev}] ${msg}`));
...
function Btn(props) {
    ...
    return (engine.createElement("button", { className: "btn btn--" + variant, onClick: onClick, disabled: disabled, "aria-disabled": disabled ? "true" : "false" }, label));
}
...
function Field(props) {
    ...
    return (engine.createElement("div", { className: "field" },
        engine.createElement("label", { htmlFor: id, className: "field__label" }, label),
        engine.createElement("input", { id: id, className: "field__input", value: value, onInput: onInput, placeholder: placeholder, "aria-label": label })));
}
// ═══════════════════════════════════════════════════════════════════════════
// UI BUILDER
// ═══════════════════════════════════════════════════════════════════════════
...
function renderPreview(item) {
    ...
    switch (item.type) {
        case "Heading":
            return engine.createElement("h" + (p.level || 2), { className: "preview-heading preview-heading--" + (p.level || 2) }, p.content || "Heading");
        case "Paragraph":
            return engine.createElement("p", { className: "preview-text" }, p.content || "Paragraph");
        case "Button":
            return engine.createElement("button", { className: "preview-btn preview-btn--" + (p.variant || "primary"), type: "button", tabIndex: -1 }, p.label || "Button");
        case "Input":
            return engine.createElement("div", { className: "preview-input-wrap" }, engine.createElement("label", { className: "preview-input-label" }, p.label || "Label"), engine.createElement("input", { className: "preview-input", placeholder: p.placeholder || "", type: "text", tabIndex: -1 }));
        case "Badge":
            return engine.createElement("span", { className: "preview-badge preview-badge--" + (p.variant || "info") }, p.content || "Badge");
        case "Divider":
            return engine.createElement("hr", { className: "preview-divider" });
        case "Div":
            return engine.createElement("div", { className: "preview-container" }, engine.createElement("span", { className: "preview-container__label" }, p.label || "Container"));
        default:
...
function PropsEditor(props) {
    ...
    if (!item) {
        return (engine.createElement("p", { className: "props-empty", "aria-live": "polite" }, "Select a canvas element to edit its properties."));
    }
    const p = item.props;
    ...
    switch (item.type) {
        case "Heading":
            return (engine.createElement("div", { className: "props-form" },
                engine.createElement(Field, { id: uid + "-content", label: "Content", value: p.content || "", onInput: (e) => onUpdate(item.id, { content: e.target.value }) }),
                engine.createElement("div", { className: "field" },
                    engine.createElement("label", { htmlFor: uid + "-level", className: "field__label" }, "Level"),
                    engine.createElement("select", { id: uid + "-level", className: "field__select", onChange: (e) => onUpdate(item.id, { level: parseInt(e.target.value) }) }, [1, 2, 3, 4, 5, 6].map(n => engine.createElement("option", { key: n, value: String(n), selected: n === (p.level || 2) }, "H" + n))))));
        case "Paragraph":
            return (engine.createElement("div", { className: "props-form" },
                engine.createElement(Field, { id: uid + "-content", label: "Content", value: p.content || "", onInput: (e) => onUpdate(item.id, { content: e.target.value }) })));
        case "Button":
            return (engine.createElement("div", { className: "props-form" },
                engine.createElement(Field, { id: uid + "-label", label: "Label", value: p.label || "", onInput: (e) => onUpdate(item.id, { label: e.target.value }) }),
                engine.createElement("div", { className: "field" },
                    engine.createElement("label", { htmlFor: uid + "-variant", className: "field__label" }, "Variant"),
                    engine.createElement("select", { id: uid + "-variant", className: "field__select", onChange: (e) => onUpdate(item.id, { variant: e.target.value }) }, ["primary", "secondary", "ghost"].map(v => engine.createElement("option", { key: v, value: v, selected: v === (p.variant || "primary") }, v[0].toUpperCase() + v.slice(1)))))));
        case "Input":
            return (engine.createElement("div", { className: "props-form" },
                engine.createElement(Field, { id: uid + "-label", label: "Label", value: p.label || "", onInput: (e) => onUpdate(item.id, { label: e.target.value }) }),
                engine.createElement(Field, { id: uid + "-placeholder", label: "Placeholder", value: p.placeholder || "", onInput: (e) => onUpdate(item.id, { placeholder: e.target.value }) })));
        case "Badge":
            return (engine.createElement("div", { className: "props-form" },
                engine.createElement(Field, { id: uid + "-content", label: "Text", value: p.content || "", onInput: (e) => onUpdate(item.id, { content: e.target.value }) }),
                engine.createElement("div", { className: "field" },
                    engine.createElement("label", { htmlFor: uid + "-variant", className: "field__label" }, "Variant"),
                    engine.createElement("select", { id: uid + "-variant", className: "field__select", onChange: (e) => onUpdate(item.id, { variant: e.target.value }) }, ["info", "success", "warning", "error"].map(v => engine.createElement("option", { key: v, value: v, selected: v === (p.variant || "info") }, v[0].toUpperCase() + v.slice(1)))))));
        case "Div":
            return (engine.createElement("div", { className: "props-form" },
                engine.createElement(Field, { id: uid + "-label", label: "Label", value: p.label || "", onInput: (e) => onUpdate(item.id, { label: e.target.value }) })));
        default:
            return engine.createElement("p", { className: "props-empty" }, "No editable properties.");
    }
}
...
function BuilderApp() {
    ...
    const addItem = (type) => {
        ...
        setItems((prev) => [...prev, { id, type, props: defaultPropsFor(type), parentId }]);
        setSelectedId(id);
        setActiveTab("canvas");
    };
    const updateItem = (id, delta) => {
        setItems((prev) => prev.map((item) => item.id === id ? Object.assign(Object.assign({}, item), { props: Object.assign(Object.assign({}, item.props), delta) }) : item));
    };
    ...
    const makeCanvasItem = (item) => {
        const nested = items.filter((i) => i.parentId === item.id);
        ...
        return engine.createElement("div", {
            ...
            onClick: (e) => { e.stopPropagation(); setSelectedId(item.id); setActiveTab("props"); },
            onKeyDown: (e) => {
        ...
        engine.createElement("div", { className: "canvas-item__header" }, engine.createElement("div", { className: "canvas-item__preview" }, renderPreview(item)), engine.createElement("span", { className: "canvas-item__type" }, isContainer
            ? (isSelected ? "Container — click Add to nest inside" : "Container")
            : item.type), engine.createElement("button", {
        ...
        isContainer && engine.createElement("div", { className: "canvas-item__children" }, nested.length === 0
            ? engine.createElement("p", { className: "canvas-item__drop-hint" }, isSelected ? "Click a component in the palette to add it here." : "Empty container")
            : nested.map((child) => makeCanvasItem(child))));
    };
    return (engine.createElement(ThemeProvider, null,
        engine.createElement("div", { className: "app", role: "application", "aria-label": "Scratch UI Builder" },
            engine.createElement("header", { className: "app-header", role: "banner" },
                engine.createElement("span", { className: "app-logo", "aria-hidden": "true" }, "\u2605"),
                engine.createElement("h1", null, "Scratch UI Builder"),
                engine.createElement("span", { className: "app-count", role: "status", "aria-live": "polite", "aria-atomic": "true" }, count + (count === 1 ? " element" : " elements"))),
            engine.createElement("nav", { className: "tab-bar", role: "tablist", "aria-label": "Builder panels" }, [
                { id: "palette", label: "Add" },
                { id: "canvas", label: "Canvas" },
                { id: "props", label: "Edit" },
            ].map(tab => engine.createElement("button", {
                key: tab.id,
                className: "tab-btn",
                role: "tab",
            ...
            engine.createElement("main", { className: "builder-main", role: "main" },
                engine.createElement("div", { id: "panel-palette", className: "panel panel--palette", role: "tabpanel", "aria-label": "Component palette", "data-active": activeTab === "palette" ? "true" : "false" },
                    engine.createElement("p", { className: "panel-title" }, selectedItem && selectedItem.type === "Div" ? "Add inside Container" : "Components"),
                    PALETTE.map(pt => engine.createElement("button", {
                        key: pt.type,
                        className: "palette-btn",
                        onClick: () => addItem(pt.type),
                        "aria-label": "Add " + pt.label,
                    }, engine.createElement("span", { className: "palette-btn__icon", "aria-hidden": "true" }, pt.icon), " " + pt.label))),
                engine.createElement("div", { id: "panel-canvas", className: "panel panel--canvas", role: "tabpanel", "aria-label": "Canvas", "data-active": activeTab === "canvas" ? "true" : "false" },
                    engine.createElement("p", { className: "panel-title" }, "Canvas" + (count > 0 ? " — " + count + (count === 1 ? " element" : " elements") : "")),
                    count === 0
                        ? engine.createElement("div", { className: "canvas-empty", "aria-live": "polite" }, engine.createElement("p", {}, "Add components from the left panel to get started."))
                        : engine.createElement("div", { className: "canvas-list", role: "listbox", "aria-label": "Canvas elements" }, items.filter((i) => !i.parentId).map((item) => makeCanvasItem(item)))),
                engine.createElement("div", { id: "panel-props", className: "panel panel--props", role: "tabpanel", "aria-label": "Element properties", "data-active": activeTab === "props" ? "true" : "false" },
                    engine.createElement("p", { className: "panel-title" }, selectedItem ? selectedItem.type + " Properties" : "Properties"),
                    engine.createElement(PropsEditor, { item: selectedItem, onUpdate: updateItem }),
                    selectedItem && (engine.createElement(Btn, { label: "Remove element", variant: "ghost-danger", onClick: () => { removeItem(selectedItem.id); setActiveTab("canvas"); } })))))));
}
// ═══════════════════════════════════════════════════════════════════════════
// BOOTSTRAP
// ═══════════════════════════════════════════════════════════════════════════
...
`);
engine.render(engine.createElement(BuilderApp, null), document.getElementById("root"));
